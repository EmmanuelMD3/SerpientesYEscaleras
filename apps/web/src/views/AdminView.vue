...
async function handleStartGame(): Promise<void> {
  ...
  try {
    ...
    errorMessage.value =
      error instanceof Error ? error.message : 'No pudimos iniciar la partida.';
  } finally {
  ...
  <main class="min-h-screen overflow-hidden bg-game px-4 py-6 text-white sm:px-6 lg:px-10">
        ...
        <div class="space-y-7">
          ...
          <div>
            ...
            <p class="mt-5 max-w-2xl text-xl font-semibold leading-8 text-white/72">
              Crea una sala, comparte el codigo y lanza preguntas simultaneas.
            </p>
          </div>
    ...
    <section v-else class="mx-auto grid min-h-[calc(100vh-3rem)] max-w-7xl gap-6 xl:grid-cols-[1fr_420px]">
          ...
          <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <h1 class="mt-3 text-4xl font-black leading-tight sm:text-6xl">
                {{ isLobby ? 'Lobby de partida' : 'Preguntas en vivo' }}
              </h1>
            </div>
            <ConnectionBadge
              :status="socketStatus"
      ...
      <aside class="grid gap-6 xl:grid-rows-[auto_1fr]">
        ...
        <section
          v-else
          class="rounded-lg border border-emerald-200/30 bg-emerald-200 p-5 text-zinc-950 shadow-glow"
        >
          <p class="text-sm font-black uppercase tracking-wide">Sala</p>
          <p class="mt-2 break-all font-mono text-5xl font-black tracking-[0.14em]">
            {{ room.code }}
          </p>
        </section>
